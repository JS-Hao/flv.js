...
class IOController {
  constructor(dataSource, config, extraData) {
    ...
    if (config.stashInitialSize != undefined && config.stashInitialSize > 0) {
      // apply from config
      console.log("最新的size", config.stashInitialSize);
  ...
  _dispatchChunks(chunks, byteStart, kbps) {
  ...
  _onLoaderChunkArrival(chunk, byteStart, receivedLength) {
    ...
    if (KBps !== 0) {
      normalized = this._normalizeSpeed(KBps);
    ...
    if (!this._enableStash) {
      ...
      if (this._stashUsed + chunk.byteLength <= this._stashSize) {
        ...
        // chunk 太大了，比 整个 _stashSize 还要大，因此只能先分配个最大（_bufferSize）的 TypeArray 去存数据
        let stashArray = new Uint8Array(this._stashBuffer, 0, this._bufferSize);
        if (this._stashUsed > 0) {
          // There're stash datas in buffer
          // dispatch the whole stashBuffer, and stash remain data
          // then append chunk to stashBuffer (stash)
          let buffer = this._stashBuffer.slice(0, this._stashUsed); // 已缓存的 bufer 数据
          let consumed = this._dispatchChunks(
            buffer,
            this._stashByteStart,
            normalized
          ...
          if (consumed < buffer.byteLength) {
            if (consumed > 0) {
              let remainArray = new Uint8Array(buffer, consumed); // 消费不完的 buffer
              stashArray.set(remainArray, 0);
    ...
    console.log("KBps", KBps, this._stashSize, this._bufferSize);
  }
