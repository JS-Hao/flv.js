...
/**
 * 在 flv header中，5 ~ 8 位 (从0位起算) 表示 dataOffset，即 flv 的 header 长度，固定为9，即
 * 00 00 00 09 (十六进制)
 * 因此，需确保传入的 bufferArray 中的第5 ~ 8位，必须为9，否则不算 flv 格式
 * 故需对以下四位字节分别左移 24、16、8、0个单位，并进行或运算，即可得出 xx xx xx xx 的格式及对应的数值
 */
...
class FLVDemuxer {
  ...
  static probe(buffer) {
    ...
    // 第四个字节表示内容标识，其中第0位与第2位分别表示 video 与 audio 的有无，因此
    // 4 = 00000100 表示 audio 的有无
    // 1 = 00000001 表示 video 的有无
    // >>> 表示无符号右移2位，确保与操作结束后，直接与 1/0 进行比较
  ...
  parseChunks(chunk, byteStart) {
    ...
    while (offset < chunk.byteLength) {
      ...
      let tagType = v.getUint8(0); // tagType 1个字节，8为Audio, 9为Video, 18为script
      let dataSize = v.getUint32(0, !le) & 0x00ffffff; // tagDataSize，3个字节，通过 & 干掉 tagType

      // 11 = 1 tagType + 3 tagDataSize + 3 timestamp + 1 timestampExtended + 3 streamId
      // 4 = previousTagSize
  ...
  // 解析 scriptData 数据
  // script tag 一般只有一个，且为 flv 的第一个 tag, 里面包含两个 AMF 包
  // AMF1：封装字符串类型数据，装入 onMetaData 标志，供 API 调用
  // AMF2: 封装数组类型，以键值对形式存储

  /**
   *
   * @param {*} arrayBuffer 本段 chunck 数据
   * @param {*} dataOffset 本段 chunck 偏移量
   * @param {*} dataSize tag Size
   */
  _parseScriptData(arrayBuffer, dataOffset, dataSize) {
    ...
    if (scriptData.hasOwnProperty("onMetaData")) {
      ...
      console.log("[jshao] 解析scriptData", this._mediaInfo);

      if (this._mediaInfo.isComplete()) {
        console.log("[jshao] 完成scriptData解析");
  ...
  /**
   * 视频解析
   * @param {*} arrayBuffer chunck
   * @param {*} dataOffset 相对于 chunck 的偏移量, tagData 的起始位置
   * @param {*} dataSize tagDataSize
   * @param {*} tagTimestamp
   * @param {*} tagPosition
   */
  _parseVideoData(
    ...
    // 240 = 11110000
    // 取第一个字节的高位，表示 frameType
    let frameType = (spec & 240) >>> 4;
    // 15 = 00001111
    // 取第一个字节的低位，表示 codecId
    let codecId = spec & 15;

    // 如果 codecId === 7, 表示它是 AVC 编码格式，会多出四个字节信息，AVCPacketType 和 CompositionTime
    // flv.js只支持 AVC 格式的解析，其实可以支持的更多:)
    if (codecId !== 7) {
      this._onError(
        DemuxErrors.CODEC_UNSUPPORTED,
  ...
  _parseAVCVideoPacket(
    ...
    // AVCPacketType 只占一个字节，CompositionTime 占三个字节
    let packetType = v.getUint8(0);
    let cts_unsigned = v.getUint32(0, !le) & 0x00ffffff;
    // 00xxxxxx --> xxxxxx00 -> 00xxxxxx
    let cts = (cts_unsigned << 8) >> 8; // convert to 24-bit signed int

    if (packetType === 0) {
      console.log("[jshao] _parseAVCDecoderConfigurationRecord");
      // AVCDecoderConfigurationRecord
      // 解析至关重要的 sps 及 pps 信息
  ...
  _parseAVCDecoderConfigurationRecord(arrayBuffer, dataOffset, dataSize) {
    ...
    // notify new metadata
    this._dispatch = false;
    console.log("[jshao] _onTrackMetadata", meta);
